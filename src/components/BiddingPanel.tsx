...
export default function BiddingPanel(
  ...
  // clamp to safe int range [0..9]
  let max = Number.isFinite(handSize) ? Math.trunc(handSize) : 0
  if (max < 0) max = 0
  if (max > 9) max = 9

  // build options without Array.from to avoid invalid length
  const opts: number[] = []
  ...
  return (
    <div className="panel" style={{ display:'flex', gap:8, alignItems:'center' }}>
      ...
      <select value={value} onChange={e => setValue(Number((e.target as HTMLSelectElement).value))}>
